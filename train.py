...
class SinusoidalPositionEmbeddings(nn.Module):
    """Embeddings sinusoïdaux pour encoder les timesteps."""
    ...
    def forward(self, timesteps: torch.Tensor):
        ...
        if self.embedding_dim % 2 == 1:
            emb = torch.cat(
                [emb, torch.zeros(timesteps.shape[0], 1, device=device)],
                dim=-1,
            )
        return emb
...
class Block(nn.Module):
    """Bloc convolutif avec injection d'embedding temporel."""
...
class UNet(nn.Module):
    """U-Net conditionné par le temps pour prédire le bruit."""
...
class LegacyBlock(nn.Module):
    """Ancien bloc convolutif simple, gardé pour compatibilité."""
...
class LegacyUNet(nn.Module):
    """Ancienne version simplifiée du U-Net."""
